...
from busylight_core.vendors.kuando._busylight import State, Step, OpCode, Ring
...
class TestKuandoBusylightStep:
    ...
    def test_step_jump_full_parameters(self):
        ...
        # Color conversion has rounding - check approximately
        retrieved_color = step.color
        assert all(abs(retrieved_color[i] - color[i]) <= 5 for i in range(3))  # Allow larger tolerance
    ...
    def test_step_color_conversion(self):
        ...
        assert all(abs(c - 128) <= 2 for c in retrieved_color)  # Allow small rounding error
...
class TestKuandoBusylightAlpha:
    ...
    def test_bytes_method(self, busylight):
        ...
        assert len(state_bytes) == 64  # State should be 7 steps + 1 footer * 8 bytes each
...
class TestKuandoBusylightKeepAlive:
    ...
    async def test_keepalive_default_interval(self, mock_light):
        ...
        with patch('asyncio.sleep') as mock_sleep:
            ...
            mock_sleep.assert_called_with(8)  # Should sleep for interval/2 rounded: round(15/2) = 8
    ...
    async def test_keepalive_interval_validation(self, mock_light):
        ...
        with pytest.raises(ValueError, match="Keepalive interval must be between 0 and 15 seconds"):
