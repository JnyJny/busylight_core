"""Tests for Agile Innovative BlinkStick Nano implementation."""
...
from busylight_core.vendors.agile_innovative import BlinkStickNano
from busylight_core.vendors.agile_innovative._blinkstick import State


class TestBlinkStickNano:
    """Test the BlinkStickNano class."""

    @pytest.fixture
    def mock_hardware(self) -> Hardware:
        """Create mock hardware for BlinkStick Nano."""
    ...
    @pytest.fixture
    def blinkstick_nano(self, mock_hardware) -> BlinkStickNano:
        """Create a BlinkStickNano instance for testing."""
        mock_hardware.handle = Mock()
        mock_hardware.handle.write = Mock(return_value=8)  # 2 + 2*3
        mock_hardware.handle.read = Mock(return_value=b"\x00" * 8)
        return BlinkStickNano(mock_hardware, reset=False, exclusive=False)

    def test_vendor_method(self) -> None:
        """Test vendor() method returns correct vendor name."""
        assert BlinkStickNano.vendor() == "Agile Innovative"

    def test_supported_device_ids(self) -> None:
        """Test supported_device_ids contains expected devices."""
        device_ids = BlinkStickNano.supported_device_ids
        assert (0x20A0, 0x41E5) in device_ids
        assert device_ids[(0x20A0, 0x41E5)] == "BlinkStick Nano"
    ...
    def test_claims_method_valid_hardware(self, mock_hardware) -> None:
        ...
        mock_hardware.release_number = 0x202

        with patch.object(BlinkStickNano.__bases__[0], "claims", return_value=True):
            assert BlinkStickNano.claims(mock_hardware) is True
    ...
    def test_claims_method_invalid_release_number(self, mock_hardware) -> None:
        ...
        with patch.object(BlinkStickNano.__bases__[0], "claims", return_value=True):
            assert BlinkStickNano.claims(mock_hardware) is False
    ...
    def test_claims_method_super_false(self, mock_hardware) -> None:
        ...
        with patch.object(BlinkStickNano.__bases__[0], "claims", return_value=False):
            assert BlinkStickNano.claims(mock_hardware) is False

    def test_state_property(self, blinkstick_nano) -> None:
        """Test state property returns BlinkStick Nano state."""
        state = blinkstick_nano.state
        assert isinstance(state, State)
        assert state.report == 6
        assert state.nleds == 2
        assert state.name == "BlinkStick Nano"
        # Should be cached
        assert blinkstick_nano.state is state

    def test_bytes_method(self, blinkstick_nano) -> None:
        """Test __bytes__ method returns state bytes."""
        state_bytes = bytes(blinkstick_nano)
        expected_bytes = bytes(blinkstick_nano.state)
        assert state_bytes == expected_bytes

    def test_on_method_all_leds(self, blinkstick_nano) -> None:
        """Test on() method with LED=0 (all LEDs)."""
        color = (255, 128, 64)
        with patch.object(blinkstick_nano, "batch_update") as mock_batch:
            mock_batch.return_value.__enter__ = Mock()
            mock_batch.return_value.__exit__ = Mock()

            blinkstick_nano.on(color, led=0)

            assert blinkstick_nano.color == color
            assert blinkstick_nano.state.color == color
            mock_batch.assert_called_once()

    def test_on_method_specific_led(self, blinkstick_nano) -> None:
        ...
        with patch.object(blinkstick_nano, "batch_update") as mock_batch:
            ...
            blinkstick_nano.on(color, led=led)

            assert blinkstick_nano.color == color
            # Check that the specific LED was set
            assert blinkstick_nano.state.get_led(led - 1) == color
            mock_batch.assert_called_once()